...
DSI::DSI(const GeoModelMesh3D& mesh) : _mesh(mesh) {
	_I = Eigen::MatrixXd::Zero(3, 4);  // I(4,3);
...
void DSI::add_constant_gradient(double w) {
	...
	for (index_t m = 0; m < _mesh.geomodel().nb_regions(); m++) {
		for (index_t c = 0; c < _mesh.cells.nb_cells(m);
			...
			for (index_t vi = 0; vi < _mesh.cells.nb_vertices(c);
			     vi++) {
				index_t vert = _mesh.cells.vertex(
				    ElementLocalVertex(c, vi));
			...
			for (index_t f = 0;
			     f < _mesh.cells.nb_facets(cell_in_gmm); f++) {
				index_t n = _mesh.cells.adjacent(
				    cell_in_gmm,
				    f);  // Global cell adjacent index
				Eigen::MatrixXd vertices_n(
				    _mesh.cells.nb_vertices(n), 3);
				for (index_t vi = 0;
				     vi < _mesh.cells.nb_vertices(n); vi++) {
					index_t vert = _mesh.cells.vertex(
					    ElementLocalVertex(n, vi));
					auto vertex =
					    _mesh.cells.mesh().vertex(vert);
					for (index_t i = 0; i < 3; i++) {
						vertices_n(vi, i) = vertex[i];
					}
				}
				Eigen::MatrixXd grad(3,3);
				calculate_cell_gradient(vertex,grad);			}
		}
	}
}
...
void DSI::calculate_cell_gradient(Eigen::MatrixXd vertices) {
	...
	for (index_t i = 0; i < 3; i++) {
		d_f(0, i) = vertices(i, 0) - vertices(0, 0);
		d_f(1, i) = vertices(i, 1) - vertices(0, 1);
		d_f(2, i) = vertices(i, 2) - vertices(0, 2);
	}
	grad = d_f.inverse() * I;
	return grad;
}
